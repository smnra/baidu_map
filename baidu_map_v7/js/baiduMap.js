...
function searchSite0(sitename){											//查找基站

	for (var i = 0; i <Point_count-1; i++) {							
		if (array_Label[i].content.indexOf(sitename)>=0) {				//遍历查找标签数组array_Label[],找出标签文本包含参数的序号
			map.centerAndZoom(array_Point[i],16)						//把地图中心定位到这个点上.
			return i 													//返回此点的序号
		};
	};
...
function searchSite2(sitename){											//查找基站 返回值为包查找字符的点的序号的数组
	siteresult=[];														//定义存放结果序号的数组
	$(".u-siterasult").text("");										//清空搜索结果框
	for (var i = 0; i <Point_count-1; i++) {							
		if (array_Label[i].content.indexOf(sitename)>=0) {				//遍历查找标签数组array_Label[],找出标签文本包含要查找字符串的序号
			siteresult.push(i)											//序号写入数组siteresult[]
			// $(".u-siterasult").text($(".u-siterasult").text()+array_Label[i].content+"<p>");		//把名字包含要查找字符串写入.u-siterasult
			$(".u-siterasult").html($(".u-siterasult").html()+"<a herf='javascript:map.centerAndZoom(array_Point[' + i +',16);'>" + array_Label[i].content + "</a>"+"<br>");
		};																//把名字包含要查找字符串写入的名字写入.u-siterasult 并加上超级链接map.centerAndZoom()
	};
	console.debug(siteresult);
	map.centerAndZoom(array_Point[siteresult[0]],16);						//把地图中心定位到这个点上.
...
function searchSite3(sitename){											//查找基站 返回值为包查找字符的点的序号的数组
	siteresult=[];														//定义存放结果序号的数组
	$(".u-siterasult").text("");										//清空搜索结果框
	for (var i = 0; i <Point_count-1; i++) {							
		if (array_Label[i].content.indexOf(sitename)>=0) {				//遍历查找标签数组array_Label[],找出标签文本包含要查找字符串的序号
			siteresult.push(i)											//序号写入数组siteresult[]
			// $(".u-siterasult").text($(".u-siterasult").text()+array_Label[i].content+"<p>");		//把名字包含要查找字符串写入.u-siterasult
			var aTag='<a href=\"javascript:map.centerAndZoom(array_Point[' + i + '],16);\">'


			$(".u-siterasult").html($(".u-siterasult").html()+aTag + array_Label[i].content + "</a><br>");
		};																//把名字包含要查找字符串写入的名字写入.u-siterasult 并加上超级链接map.centerAndZoom()
	};
	//console.debug(siteresult);
	map.centerAndZoom(array_Point[siteresult[0]],16);						//把地图中心定位到这个点上.
